...
function Card({ index, type, gameTitle, subTitle, statData, gameId, gameWords }) {
  const dispatch = useDispatch();
  const position = (index * 5) + "%";
  const returnCard = () => {
    if (type === "game") {
      return (
        <div className={styles.gameCard} style={{backgroundPositionX: position}}>
          <div className={styles.play}>
            <PlayButtonIcon />
            <span>Play</span>
          </div>
          <h3>{gameTitle}</h3>
          <h5>{subTitle} total matching groups</h5>
